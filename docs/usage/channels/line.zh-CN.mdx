...
<Steps>
  ...
  打开 **Basic settings** 选项卡，复制以下两个值供第四步使用：

  - **Channel secret** —— 用于校验 Webhook 签名。
  - **Your user ID** —— 这个 channel 对应的 bot 用户 ID（以 `U` 开头，共 33 位）。LINE 官方文档说明：*"You can access your user ID under the Basic settings tab of your channel."* 这就是 LobeHub 需要的 **Destination User ID**，也是每条入站 webhook payload 中的 `destination` 字段。
...
<Steps>
  ...
  | 字段                       | 值                                           |
  | ------------------------ | ------------------------------------------- |
  | **Destination User ID**  | Basic settings 选项卡中的 **Your user ID**（`U…`） |
  | **Channel Access Token** | 第二步中签发的长期 Token                             |
  | **Channel Secret**       | Basic settings 选项卡中的 Channel secret         |
...
| 字段                       | 是否必需 | 描述                                                                                    |
| ------------------------ | ---- | ------------------------------------------------------------------------------------- |
| **Destination User ID**  | 是    | 机器人的用户 ID（`U` + 32 位十六进制），来自 **Basic settings → Your user ID**。同时作为机器人标识与 Webhook 路径。 |
...
- **`Channel access token belongs to bot Uxxx, not Uyyy`。** Destination User ID 与 Token 不匹配。打开 **Basic settings → Your user ID**，把准确的 `U…` 值填进 Destination User ID 字段。
